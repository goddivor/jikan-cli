import { JikanResponse, JikanSingleResponse, JikanMangaResponse, JikanSingleMangaResponse, AdvancedSearchOptions } from "../types/anime";
...
export class JikanApi {
  ...
  static async searchManga(
    ...
    let url = `${this.BASE_URL}/manga?q=${encodeURIComponent(
      query
    ...
    if (advancedOptions?.genres && advancedOptions.genres.length > 0) {
      const genreIds = this.getMangaGenreIds(advancedOptions.genres);
      if (genreIds.length > 0) {
        url += `&genres=${genreIds.join(',')}`;
      }
    }
  ...
  static async searchMangaByGenre(
    genres: string[],
    limit: number = 25,
    excludeGenres?: string[],
    orderBy?: string,
    ...
    const genreIds = this.getMangaGenreIds(genres);
    let url = `${this.BASE_URL}/manga?limit=${limit}`;
    ...
    if (excludeGenres && excludeGenres.length > 0) {
      const excludeIds = this.getMangaGenreIds(excludeGenres);
      if (excludeIds.length > 0) {
        url += `&genres_exclude=${excludeIds.join(',')}`;
      }
    }
  ...
  private static getMangaGenreIds(genreNames: string[]): number[] {
    const mangaGenreMap: { [key: string]: number } = {
      'action': 1, 'adventure': 2, 'comedy': 4, 'drama': 8,
      'fantasy': 10, 'horror': 14, 'romance': 22, 'sci-fi': 24,
      'slice of life': 36, 'supernatural': 37, 'thriller': 41,
      'ecchi': 9, 'school': 26, 'historical': 23, 'mecha': 18,
      ...
      'space': 29, 'super power': 31, 'vampire': 32, 'sports': 30,
      'military': 38, 'police': 39, 'psychological': 40,
      'seinen': 42, 'shoujo': 43, 'shounen': 27, 'josei': 28
    };

    return genreNames
      .map(name => mangaGenreMap[name.toLowerCase()])
      .filter(id => id !== undefined);
  }

  static async getMangaById(id: string): Promise<JikanSingleMangaResponse> {
    const url = `${this.BASE_URL}/manga/${encodeURIComponent(id)}`;
    const response = await fetch(url);
    return await response.json();
  }

  static async getTopMangas(
    limit: number,
    orderBy?: string,
    sortOrder?: string
  ): Promise<JikanMangaResponse> {
    let url = `${this.BASE_URL}/top/manga?limit=${limit}`;
  ...
  static async getRandomManga(): Promise<JikanSingleMangaResponse> {
    const url = `${this.BASE_URL}/random/manga`;
    const response = await fetch(url);
    return await response.json();
  }
}
